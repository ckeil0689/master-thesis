...
# rearrange the mtls in order of mtl size and alphanumeric
ix <- numeric()
l.vec <- sapply(names(c.table.th17),function(i) length(strsplit(i,"_")[[1]]))
for(i in 1:length(sort(unique(l.vec)))){
	ix <- c(ix,which(l.vec == i))
}
c.table.th17 <- c.table.th17[ix]
...
for(i in 1:length(c.table.th17)){
  ...
  pvals.th17[[type]] <- as.numeric(unlist(sapply(ll.th17[ix],function(i) i[["pval"]]/10 )))
  for(j in 1:length(tfs)){
	tf <- tfs[j]
  	pvals.th17.per.tf[[tf]][[type]] <- as.numeric(unlist(sapply(ll.th17[ix],function(i) i[["pval"]][which(i$tf.to.s==tf)]/10 )))
  }
...
# rearrange the mtls in order of mtl size and alphanumeric
ix <- numeric()
l.vec <- sapply(names(c.table.th0),function(i) length(strsplit(i,"_")[[1]]))
for(i in 1:length(sort(unique(l.vec)))){
	ix <- c(ix,which(l.vec == i))
}
c.table.th0 <- c.table.th0[ix]
...
for(i in 1:length(c.table.th0)){
  ...
  pvals.th0[[type]] <- as.numeric(unlist(sapply(ll.th0[ix],function(i) i[["pval"]]/10 )))
  for(j in 1:length(tfs)){
	tf <- tfs[j]
  	pvals.th0.per.tf[[tf]][[type]] <- as.numeric(unlist(sapply(ll.th0[ix],function(i) i[["pval"]][which(i$tf.to.s==tf)]/10 )))
  }
  ## add histone marks pvals
