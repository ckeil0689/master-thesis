...
## /|/ \|\ /|/ \|\ /|/ \|\ /|/ \|\ /|/ \|\ /|/ \|\ / / \ \ / / \ \
##`-'   `-`-'   `-`-'   `-`-'   `-`-'   `-`-'   `-`-'   `-`-'   ' '
## June 2011 Th17 project (MCZ,tlCLR,Inferelator)
...
path.output <- "results/"
path.input.counts <- "input/th17/used_for_paper/rawData/rnaseq_ht_counts_July_5_2011/"
...
expt.mat <- read.delim(file=paste(sep="",path.input,"diffExpressionExpts_july_5_2011.txt"),colClasses = "character")

## for(i in 1:length(expts)){
for(i in 1:nrow(expt.mat)){
  ## e.wt <- strsplit(strsplit(expts[i],"::")[[1]],"_")[[1]]
  ## e.ko <- strsplit(strsplit(expts[i],"::")[[1]],"_")[[2]]
  ## e.nm <- expts.names[i]
  ...
  cat("working on",e.nm,"\n")
  f.nm.wt<- sapply(e.wt,function(i) grep(i,count.files,value=T))
  f.nm.ko<- sapply(e.ko,function(i) grep(i,count.files,value=T))
  if(i == 1){
    x <- read.delim(file=paste(sep="",path.input.counts,f.nm.wt[1]),header=T)
    n.row <- dim(x)[1]
    gn.nms <- toupper(as.character(x[,1]))
    s.mat.padj <- matrix(0,nc=(1+nrow(expt.mat)),nr=length(gn.nms),dimnames=list(gn.nms,c("id",expt.mat[,1])))
    s.mat.log2fc <- matrix(0,nc=(1+nrow(expt.mat)),nr=length(gn.nms),dimnames=list(gn.nms,c("id",expt.mat[,1])))
    s.mat.padj[,1] <- gn.nms
  ...
  s.mat.padj[,i+1] <- res$padj
  s.mat.log2fc[,i+1] <- res$log2FoldChange
  write.table(res,file=paste(path.results,e.nm,".xls",sep=""),sep="\t",row.names=FALSE)
}
write.table(s.mat.padj,file=paste(path.results,"summary.pval.adjusted.xls",sep=""),sep="\t",row.names=FALSE)
write.table(s.mat.log2fc,file=paste(path.results,"summary.log2fc.xls",sep=""),sep="\t",row.names=FALSE)
