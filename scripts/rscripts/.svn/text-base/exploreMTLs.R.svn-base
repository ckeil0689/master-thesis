...
path.output <- "results/fig1/"

# file names
f.nm.ll.th17 <- paste(sep="",path.mtl.list,"tf_clusters_th17_100_Nov_17_2011.RData")
f.nm.ll.th0 <- paste(sep="",path.mtl.list,"tf_clusters_th0_100_Nov_17_2011.RData")
...
pdf(file=paste(sep="",path.output,"5_way_MTL_tf_positions_",date.is,".pdf"))
par(mfrow=c(2,3))
for(i in 1:length(core.tfs)){
	tf <- core.tfs[i]
...
pdf(file=paste(sep="",path.output,"5_way_MTL_tf_pval_per_position_",date.is,".pdf"))
par(mfrow=c(2,3))
for(i in 1:length(core.tfs)){
	tf <- core.tfs[i]
...
cut.th17 <- 500
ix.th17 <- which(mtl.num.total > cut.th17)
...
# stop("AM")
pdf(file=paste(sep="",path.output,"mtls_and_histone_marks_",date.is,".pdf"))
par(mfrow=c(2,2))
main="MTL classification (proximal or distal)"
ylab="#"
mp <- barplot(t(x[ix.th17,]),beside=T,axes = FALSE,axisnames = FALSE,main=main,ylab=ylab,cex.main=.9)
axis(2,cex.axis=0.7)
# axis(4)
n=3
text(seq(2,n*length(ix.th17),by=n), par("usr")[3], labels = rownames(x)[ix.th17], srt = 45, adj = c(1,1.5), xpd = TRUE,cex=.3)
legend("topright",colnames(x[ix.th17,]),fill=c("darkgray","white"),cex=.7)

main="MTL classification (normalized)"
ylab="proportion"
mp <- barplot(t(x[ix.th17,]/apply(x[ix.th17,],1,sum)),beside=T,axes = FALSE,axisnames = FALSE,ylim=c(0,1),main=main,ylab=ylab,cex.main=.9)
axis(2,cex.axis=0.7)
# axis(4)
n=3
text(seq(2,n*length(ix.th17),by=n), par("usr")[3], labels = rownames(x)[ix.th17], srt = 45, adj = c(1,2), xpd = TRUE,cex=.3,cex.main=.9)
...
# axis(4)
n=3
text(mp, par("usr")[3], labels = mtl.types, srt = 45, adj = c(1,1.5), xpd = TRUE,cex=.5)
...
axis(2,cex.axis=0.7)
axis(4)
text(1:length(sam.list), par("usr")[3], labels = names(sam.list), srt = 45, adj = c(1,1.5), xpd = TRUE,cex=.5)
