...
get.one.comb.type <- function(res,score.tp="whole",chip.tp="th17",extract="KC"){
	...
	extra.extract <- c("SAM","th0_rpkm","th17_rpkm")
	m <- matrix(0,nr=nrow(res[[score.tp]][[1]][[chip.tp]]),nc=length(tfs)+length(extra.extract))
	colnames(m) <- c(tfs,extra.extract)
	...
	# add extras
	for(i in 1:length(extra.extract)){
		extra <- extra.extract[i]
		m[,extra] <- res[[score.tp]][[1]][[chip.tp]][,extra]
	}
