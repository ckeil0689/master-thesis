...
for(i in 1:length(tfs.to.cluster)){
	tmp.th17 <- c(tmp.th17,keepers.th17[grep(tfs.to.cluster[i],tfs.th17,ignore.case=T)])
	tmp.th0 <- c(tmp.th0,keepers.th0[grep(tfs.to.cluster[i],tfs.th0,ignore.case=T)])
}
...
# f.nm <- paste(sep="",path.input.save.results,"tf_clusters_",paste(tfs,collapse="_"),"_th17_d.tfs_",d.cut,"_",date.is,".RData")
...
# f.nm <- paste(sep="",path.input.save.results,"tf_clusters_",paste(tfs,collapse="_"),"_th0_d.tfs_",d.cut,"_",date.is,".RData")
