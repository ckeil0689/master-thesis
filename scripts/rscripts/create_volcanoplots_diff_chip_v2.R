...
# output at end a mtl list for th0 and a th17 with all comparisons
# ll.th17.for.output <- ll.th17.all.mtls
# ll.th0.for.output <- ll.th0.all.mtls
#####################################################################
...
for(i in 1:length(expts)){ # for each diff chip expt
	...
	for(k in 1:rep.num){
		...
		e.fc <- paste(tf.chip,"_FC_",sl.wt,"_",sl.ko,"_rep_",k,sep="")
		e.pval <- paste(tf.chip,"_pval_",sl.wt,"_",sl.ko,"_rep_",k,sep="")
		for(l in 1:length(ll)){			
			if(wt[l]>=ko[l]){
				pval <- -1*ppois(wt[l],lambda=lambda.wt[l],lower.tail=FALSE,log.p = TRUE)/log(10)
				# ll[[l]][[e.pval]] <- -1*ppois(wt[l],lambda=lambda.wt[l],lower.tail=FALSE,log.p = TRUE)/log(10)
			} else {
				pval <- -1*ppois(ko[l],lambda=lambda.ko[l],lower.tail=FALSE,log.p = TRUE)/log(10)
				# ll[[l]][[e.pval]] <- -1*ppois(ko[l],lambda=lambda.ko[l],lower.tail=FALSE,log.p = TRUE)/log(10)
			}
			ll[[l]][[e.fc]] <- fc[l]
			ll[[l]][[e.pval]] <- pval
			if(tolower(cond)==tolower("Th17")){
				ll.th17.all.mtls[[l]][[e.fc]] <- fc[l]
				ll.th17.all.mtls[[l]][[e.pval]] <- pval
			} else {
				ll.th17.all.mtls[[l]][[e.fc]] <- fc[l]
				ll.th17.all.mtls[[l]][[e.pval]] <- pval
			}
		...
		ix.fc <- grep( paste(sep="","FC","_",sl.wt,"_",sl.ko),names(ll[[1]]) )
		ix.pval <- grep( paste(sep="","pval","_",sl.wt,"_",sl.ko),names(ll[[1]]) )
		...
		# plot volcano plot with highlights
		main <- paste(sep="",tf.chip," ChIP ","(in ",cond,"-",tf.ko," WT/KO ",sl.wt,"/",sl.ko,") [highlight]")
		# stop("AM")
		for(l in 1:2){
			if(l == 1){ix.focus <- ix.distal;pch=20;cex=.3} else {ix.focus <- ix.proximal;pch=3;cex=.3}
			for(j in length(ix.mtls):1){
				ix <- intersect(ix.mtls[[j]],ix.focus)
				if(l==1&j==length(ix.mtls)){
					plot(fc[ix],pval[ix],cex=cex,xlab=xlab,ylab=ylab,xlim=xlim,ylim=ylim,main=main,pch=pch,col=col.vec[j],xaxt="n",cex.axis=2)
					axis(1,at=seq(xlim[1],xlim[2],by=1),cex.axis=1.7)	
				} else {
					points(fc[ix],pval[ix],cex=cex,pch=pch,col=col.vec[j])
				}
			}
		}
		lines(x=c(-10,10),y=c(pval.cut,pval.cut),lwd=2)
		lines(x=c(-1*fc.cut,-1*fc.cut),y=c(-10,160),lwd=2)
		lines(x=c(fc.cut,fc.cut),y=c(-10,300),lwd=2)
		legend(x=xlim[1]-0.3,y=ylim[2]+6,paste(sep="",names(ix.mtls)," (n=",sapply(ix.mtls,length),")"),fill=col.vec,cex=1,title="MTL type")		
		if(length(ix.highlight)>0){
			...
			text(x=fc[ix.highlight],y=pval[ix.highlight]-4,trgts[ix.highlight],col="black",cex=1)
			# put % numbers on right hand side of volcano plot
			prcnt.sig.fc.and.pval <- length(which(pval>pval.cut & fc>fc.cut))/length(pval)*100
			prcnt.sig.fc.not.pval <- length(which(fc>fc.cut))/length(fc)*100-prcnt.sig.fc.and.pval
			text(x=5,y=130,paste(sep="",round(prcnt.sig.fc.and.pval,2),"%"),col="black",cex=2)
			text(x=5,y=3,paste(sep="",round(prcnt.sig.fc.not.pval,2),"%"),col="black",cex=2)
			# put % numbers on left hand side of volcano plot
			prcnt.sig.fc.and.pval <- length(which(pval>pval.cut & fc< (-fc.cut) ))/length(pval)*100
			prcnt.sig.fc.not.pval <- length(which(fc < (-fc.cut) ))/length(fc)*100-prcnt.sig.fc.and.pval
			text(x=-2.3,y=130,paste(sep="",round(prcnt.sig.fc.and.pval,2),"%"),col="black",cex=2)
			text(x=-2.3,y=3,paste(sep="",round(prcnt.sig.fc.not.pval,2),"%"),col="black",cex=2)
		}

		# plot volcano plot without highlights
		main <- paste(sep="",tf.chip," ChIP ","(in ",cond,"-",tf.ko," WT/KO ",sl.wt,"/",sl.ko,") [no highlight]")
		for(l in 1:2){
			if(l == 1){ix.focus <- ix.distal;pch=20;cex=.3} else {ix.focus <- ix.proximal;pch=3;cex=.3}
			for(j in length(ix.mtls):1){
				ix <- intersect(ix.mtls[[j]],ix.focus)
		...
		ylim <- c(0,100)
		mp <- boxplot(pval.list,outline=FALSE,col=col.vec,xaxt="n",yaxt="n",notch=T,ylim=ylim)
		axis(2,at=seq(ylim[1],ylim[2],by=10),cex.axis=1.7)	
		axis(1,at=1:length(mp$n),label=paste(sep="","n=",mp$n))
		ylim <- c(-3,5)
		mp <- boxplot(fc.list,outline=FALSE,col=col.vec ,xaxt="n",yaxt="n",notch=T,ylim=ylim)	
		axis(2,at=seq(ylim[1],ylim[2],by=1),cex.axis=1.7)
		axis(1,at=1:length(mp$n),label=paste(sep="","n=",mp$n))
	}
	dev.off()
}
f.nm <- paste(sep="",path.results,"th17_wt_vs_ko_min_mtl_pval_",mtl.min.pval,"_",date.is,".xls")
print.ll.verbose.all(ll.th17.all.mtls,f.nm=f.nm)
f.nm <- paste(sep="",path.results,"th0_wt_vs_ko_min_mtl_pval_",mtl.min.pval,"_",date.is,".xls")
print.ll.verbose.all(ll.th0.all.mtls,f.nm=f.nm)
